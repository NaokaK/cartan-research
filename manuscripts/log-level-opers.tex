...
すなわち $\sP$ 上には反例が存在しない。
(3) 反例は marked point が無い場合（$D=\emptyset$）に集中し、
そこでは $\Pic$ の $p$ 可除性が障害となる。
...
後半：$N=1$ のとき $\lambda_x$ は $(0,p)$ 内の奇数（$p\nmid\lambda_x$ は自動）。
三角不等式から $2\lambda_0<\sum\lambda_x<2p$ すなわち $\lambda_x<p$ は自動的に満たされる。
$\lambda_x=2m_x+1$、$0\le m_x\le(p-3)/2$ と置き、
$n:=(p-1)/2$ とすると、条件は
$m_x\le m_y+m_z$（巡回）かつ $\sum m_x\le n-1$ 相当に翻訳され、
数え上げは四角錐数 $n(n+1)(2n+1)/6=p(p^2-1)/24$ を与える。
\Todo{この組合せ論的同一視をきちんと書く。現状は下表の数値実験による確認。}
\end{proof}

\begin{table}[htbp]
\centering
\caption{命題 \ref{prop:count} の数値確認（計算機による直接列挙）}
\label{tab:count}
\begin{tabular}{lrrrrr}
\toprule
$p$ & 3 & 5 & 7 & 11 & 13\\
\midrule
$\#\,{}^1\Op^{\Zzz}_{2,\sP}$（列挙）& 1 & 5 & 14 & 55 & 91\\
$p(p^2-1)/24$ & 1 & 5 & 14 & 55 & 91\\
\midrule
$\#\,{}^2\Op^{\Zzz}_{2,\sP}$（列挙）& --- & 610 & 2262 & --- & ---\\
\bottomrule
\end{tabular}
\end{table}

\begin{remark}
$N=2$ の個数には $p\nmid\lambda_x$ の条件が効くため、
\eqref{eq:count1} で $p\to p^N$ と置き換えた式にはならない
（$p=5$：$25\cdot624/24=650\neq610$、差 $40$）。
閉じた公式は未解決（\S\ref{sec:open}）。
\end{remark}
...
$(0,5)$ 内の奇数は $1,3$ のみなので、
条件 \eqref{eq:triangle} と $\sum\lambda_x<10$ を満たす三つ組は
\[
  (1,1,1),\quad(3,1,3),\quad(1,3,3),\quad(3,3,1),\quad(3,3,3)
\]
の $5$ 個である。除外されるのは $(1,1,3)$ 型（$3<1+1$ に反する）。
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{$\sP$ 上には反例が存在しない}\label{sec:M03}

\begin{theorem}\label{thm:M03}
$\sP=(\bbP^1;0,1,\infty)$ とする。
$\cD^{(N-1)}\subset\cD^{(N)}$ による制限写像
\[
...
\begin{proof}
定理 \ref{thm:covering} により、レベル $N$ の対象は
「$\lambda_x$ 奇数、$p\nmid\lambda_x$、三角不等式 \eqref{eq:triangle}、
$\sum\lambda_x<2p^N$」を満たす三つ組と一対一に対応する。
制限写像は radius を $\bmod\,p^N$ に落とす、
すなわち補題 \ref{lem:lambda-unique} により
$\lambda'\mapsto$「$\pm\lambda'$ の $(0,p^N)$ 内の奇代表」を与える。
レベル $N$ で許容な $(\lambda_0,\lambda_1,\lambda_\infty)$ に対し、
$\lambda'_x:=\lambda_x$ とおけば
$\sum\lambda'_x<2p^N<2p^{N+1}$ であり、
他の条件も不変なのでレベル $N+1$ で許容であり、
かつ $\mathrm{res}_N$ で元に戻る。
\end{proof}

\begin{example}
表 \ref{tab:p5} の \#5（$\lambda=(3,3,3)$）のレベル $2$ への持ち上げ先は、
$\lambda'_x\equiv\pm3\pmod5$、$\lambda'_x$ 奇数、$5\nmid\lambda'_x$、$0<\lambda'_x<25$
から $\lambda'_x\in\{3,13,23\}\cup\{7,17\}$ を選び、
三角不等式と $\sum\lambda'_x<50$ を満たすものすべて。
$\lambda'=(3,3,3)$（すなわち同じ被覆）が常に候補となる。
\end{example}

\begin{remark}
定理 \ref{thm:M03} は系 \ref{cor:obstruction} の $r\ge1$ の場合の
完全な実現である。命題 \ref{prop:rank1curve}（階数 $1$）と合わせ、
\textbf{対数構造（marked point）は持ち上げの障害を消す方向に働く}
という本稿の主張を支持する。
\end{remark}
...
6 & $g\ge2$, $r=0$ & dormant $\mathrm{PGL}_2$-oper（$F^N$-射影構造）
  & --- & \textbf{未決定} & 系 \ref{cor:obstruction} の $r=0$\\
\bottomrule
\end{tabular}
\end{table}
...
\begin{question}[本命：$r=0$ の場合]\label{q:main}
種数 $g\ge2$ の非点付き曲線上で、
$F^1$-射影構造であって $F^2$-射影構造に持ち上がらないものを構成せよ。
系 \ref{cor:obstruction} は $r=0$ で自由度が消えることを示しているが、
実際に $\mathrm{res}_1$ が非全射となる $(g,p)$ を特定する必要がある。
$F^N$-射影構造の個数公式（Joshi--Vologodsky 型、Hoshi, Wakabayashi）を
比較し $|{}^2\Op|<|{}^1\Op|$ となる場合を探すのが最短と思われる。
\end{question}

\begin{question}[次数障害の十分性]
命題 \ref{prop:degree} の合同式 \eqref{eq:degcong} は必要条件である。
$g\ge1$, $r\ge1$ において、これが（適当な安定性の下で）十分条件となるか。
$(g,r)=(0,3)$ では定理 \ref{thm:M03} が肯定的に答える。
\end{question}

\begin{question}[レベル $N$ の数え上げ公式]
$\#\,{}^N\Op^{\Zzz}_{2,\sP}$ の閉じた公式を求めよ。
$N=1$ では $p(p^2-1)/24$（命題 \ref{prop:count}）。
$N=2$, $p=5$ では $610$、$p=7$ では $2262$（表 \ref{tab:count}）。
$p\nmid\lambda_x$ の条件が包除原理的な補正項を生む。
\end{question}

\begin{question}[高次元]
$D$ が SNC のとき exponent は $(\bbZ/p^N)^r$ に値をとる。
交差 $D_i\cap D_j$ における両立条件が新しい障害を生むか。
Ohkawa の indexed algebra の枠組みで定式化できるはず。
\end{question}

\begin{question}[一般 $(G,P)$]
radius は $\mu_{p^N}\to T$ の $W$-共役類として定義されるべきである。
$\pi_1(G)$ の $p$-捩れが命題 \ref{prop:degree} の次数障害に相当するものを与えるか。
$\mathrm{PGL}_n$ では $\pi_1=\bbZ/n$ であり、$\gcd(n,p)$ による場合分けが予想される。
\end{question}
