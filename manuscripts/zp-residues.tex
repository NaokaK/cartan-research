...
$\sigma_n$}.  For $c\not\equiv c'\pmod\ZZ$ we have $\Hom(\cL_c,\cL_{c'})=0$:
a nonzero such map, being a nonzero $K$-linear map between rank-one
objects, is an isomorphism $\cO\cong\cL_{c'-c}$; transporting the lattice
of $\cL_{c'-c}$ along it gives a logarithmic lattice of the trivial bundle
$\cO$ with $r$-spectrum $\{c'-c\}$, but any two $R$-lattices in a
$1$-dimensional $K$-vector space differ by a power of $t$, i.e.\ every
logarithmic lattice of $\cO$ is a Hecke modification of the standard one
by some $\nu\in\Xst=\ZZ$, so by \Cref{thm:welldef}(2) its $r$-spectrum
lies in $\ZZ$ -- contradicting $c'-c\notin\ZZ$.  For $c\equiv c'\pmod\ZZ$
(including $c=c'$), $\cL_{c'-c}\cong\cO$ (\Cref{rem:completemodZ}), so
$\Ext^1(\cL_c,\cL_{c'})\cong\Ext^1(\cO,\cO)$, which vanishes by
\Cref{cor:allzero}: a self-extension has $r=0$, hence is trivial.
Therefore the filtration splits, with graded pieces belonging to the same
class mod $\ZZ$ merged into isomorphic copies of a single $\cL_c$ as in
\Cref{rem:completemodZ}.
...
The corrected statement, and the corresponding fix to the Hom/Ext
argument in the proof of \Cref{thm:localstructure} (which had implicitly
assumed $\Hom(\cL_c,\cL_{c'})=0$ for all $c\ne c'$, true only for
$c\not\equiv c'\pmod\ZZ$), are given in \Cref{rem:completemodZ} and in the
proof of \Cref{thm:localstructure} itself.  This correction does not
